...
type (
	...
	Publisher struct {
		...
		quit       *internal.DoneChan
		pauseChan  chan internal.PlaceholderType
		resumeChan chan internal.PlaceholderType
	}
)
...
func NewPublisher(endpoints []string, key, value string, opts ...PublisherOption) *Publisher {
	publisher := &Publisher{
		...
		key:        key,
		value:      value,
		quit:       internal.NewDoneChan(),
		pauseChan:  make(chan internal.PlaceholderType),
		resumeChan: make(chan internal.PlaceholderType),
	}
...
func (p *Publisher) Pause() {
	p.pauseChan <- internal.Placeholder
}

// Resume resumes the renewing of key:value.
func (p *Publisher) Resume() {
	p.resumeChan <- internal.Placeholder
}
			...
			case _, ok := <-ch:
				if !ok {
					...
					if err := p.KeepAlive(); err != nil {
						log.Fatalf("KeepAlive: %s", err.Error())
					}
					return
				}
			case <-p.pauseChan:
				log.Printf("paused etcd renew, key: %s, value: %s", p.key, p.value)
				p.revoke(cli)
				select {
				case <-p.resumeChan:
					if err := p.KeepAlive(); err != nil {
