...
func (c *cluster) reload(cli EtcdClient) {
	...
	close(c.done)
	c.watchGroup.Wait()
	c.done = make(chan PlaceholderType)
	c.watchGroup = new(stack.Group)
	var keys []string
...
func (c *cluster) watch(cli EtcdClient, key string) {
	...
	for {
		...
		case wresp, ok := <-rch:
			if !ok {
				log.Printf("etcd monitor chan has been closed")
				return
			}
			if wresp.Canceled {
				log.Printf("etcd monitor chan has been canceled")
				return
			}
			if wresp.Err() != nil {
				log.Printf(fmt.Sprintf("etcd monitor chan error: %v", wresp.Err()))
				return
			}
